...
>	grep -E "[[:alnum:]]\*([^[:digit:]]){1,}([[:alnum:]])\*" file
3.	Que acabe con un signo != a '^'
>	grep -E "[^\^]$" file

**grep -E "^([[:upper:]])[[:alnum:]]\*([^[:digit:]]){1,}([[:alnum:]])\*[^\^]$" ejsPrueba/file1.txt**

##	2.	Expresión regular que utilizarías para encontrar las líneas que acabasen con dos números:
>	grep -E "^([^[:digit:]]\*[[:digit:]]{2})$" ejsPrueba/file2.txt
